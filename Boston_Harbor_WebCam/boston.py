...
folderText = Text(value = '/Ship01/Dataset/water_collection/')
intervalSlider = IntSlider(value = 10, min = 1, max = 60, step = 1)
totalTimeSlider = IntSlider(value = 24, min = 1, max = 180, step = 1)
downloadBtn = Button(description = 'Download')
timezoneList = Dropdown(value='America/New_York', options=pytz.all_timezones) 
...
def download_btn_clicked(a):
    ...
    print(int(totalTime*60/interval))
    ...
    for gap in range (int(totalTime*60/interval)):
        ...
        process('http://108.49.186.56/axis-cgi/mjpg/video.cgi?resolution=1024x768&dummy=1510881269401', rootfolder, 'Boston_Tea_Party_Museum_Webcam_2_202006')
...
def process(imageurl, folder, name):
    ...
    try:
        ...
        imageName = now_time.strftime('%Y-%b-%d-%H-%M-%S') + '.jpg'
        ...
        draw.text(pos, imageName[:-4] + ' ' + timezoneList.value, font = font)
